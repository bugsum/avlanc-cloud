...
export async function POST(request: Request) {
  try {
    ...
    const { merchantOrderId, transactionId, state, code, message } = callbackData;
    ...
    switch (state) {
      case 'COMPLETED':
        redirectUrl = `${baseUrl}/payment/success?orderId=${encodeURIComponent(merchantOrderId)}`;
        if (transactionId) {
          redirectUrl += `&transactionId=${encodeURIComponent(transactionId)}`;
        }
        break;

      case 'PENDING':
        redirectUrl = `${baseUrl}/payment/pending?orderId=${encodeURIComponent(merchantOrderId)}`;
        if (transactionId) {
          redirectUrl += `&transactionId=${encodeURIComponent(transactionId)}`;
        }
        break;
      ...
      case 'FAILED':
      default:
        redirectUrl = `${baseUrl}/payment/failed?orderId=${encodeURIComponent(merchantOrderId)}`;
        if (code) {
    ...
    const redirectUrl = `${baseUrl}/payment/error?message=${encodeURIComponent(errorMessage)}`;
